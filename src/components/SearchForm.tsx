...
export default function SearchForm({ onNext, isLoading = false }: SearchFormProps) {
  ...
  const inputCls = "w-full px-4 py-3.5 rounded-[8px] border border-[#d6dbde] bg-white text-black/87 focus:outline-none focus:border-[#00754A] focus:ring-1 focus:ring-[#00754A] transition placeholder:text-black/30 disabled:opacity-50 text-sm";

  return (
    <form
      onSubmit={handleSubmit}
      className="bg-white p-8 rounded-[12px] w-full max-w-4xl mx-auto flex flex-col gap-6"
      style={{ boxShadow: '0 0 0.5px rgba(0,0,0,0.14), 0 2px 8px rgba(0,0,0,0.10)' }}
    >

      {/* 치과명 */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <label className="block text-sm font-semibold text-black/87">치과 풀네임 <span className="text-[#c82014]">*</span></label>
        ...
        <div className="space-y-2">
          <label className="block text-sm font-semibold text-black/87">단축명 <span className="text-black/40 font-normal">(선택)</span></label>
      ...
      {/* 진료과목 3개 박스 */}
      <div className="space-y-2">
        <label className="block text-sm font-semibold text-black/87">
          진료과목 <span className="text-[#c82014]">*</span>
          <span className="text-black/40 font-normal ml-1">최대 3개</span>
        </label>
      ...
      <div className="space-y-2">
        <label className="block text-sm font-semibold text-black/87">
          지역명 <span className="text-[#c82014]">*</span>
          <span className="text-black/40 font-normal ml-1">최대 3개</span>
        </label>
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1">
