...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-[family-name:var(--font-geist-sans)] pb-32 text-slate-100 overflow-hidden relative">
      ...
      <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full bg-green-900/40 blur-[150px] pointer-events-none" />
      ...
      <header className="border-b border-white/10 bg-slate-900/50 backdrop-blur-md sticky top-0 z-50 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="bg-gradient-to-tr from-emerald-500 to-green-600 p-2 rounded-xl shadow-lg shadow-emerald-500/20">
            <Activity className="text-white w-6 h-6" />
          </div>
          <h1 className="text-xl font-bold tracking-tight text-white">
            닥터원츠 <span className="text-emerald-400">GEO</span> 프로그램
          </h1>
        </div>
            ...
            <div key={s} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                ...
                ['loading', 'results'].includes(step) && i < ['input','prompts','loading'].indexOf(step)
                  ? 'bg-emerald-600/30 text-emerald-400' : 'bg-slate-800 text-slate-500'
              }`}>{i + 1}</div>
          ...
          {step === 'input' && (
            <motion.div key="input" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="w-full space-y-10">
              <div className="text-center space-y-4">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 text-sm font-semibold">
                  <MapPin className="w-4 h-4" />
                  실제 환자들의 로컬 검색점유율 분석
                </div>
                <h2 className="text-4xl md:text-5xl font-extrabold text-white tracking-tight leading-tight">
                  치과 AI 플랫폼 검색 장악력을<br className="hidden md:block" />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400"> 한눈에 확인하세요</span>
                </h2>
          ...
          {step === 'loading' && (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center py-32 space-y-6">
              ...
              <div className="text-center">
                <p className="text-emerald-400 font-bold text-xl">{loadingMsg}</p>
